...
public class SpecialEffectManager : MonoBehaviour
{
    ...
    public float GetOnServePressurePrediction(CardData card)
    {
        ...
        {
            case "SnailNoodle": return 20f; //螺蛳粉
            case "BeefBall": return 15f;    //撒尿牛丸
            case "GhostPepper": return 25f; //魔鬼辣椒
            case "DryIce": return -20f;     //干冰
            case "WellingtonSlipper": return 5f; //惠灵顿拖鞋
            case "SmileMadness": return 30f; //含笑半步癫
            case "RedUmbrella": return 15f;  //红伞伞
            case "Intestine": return 10f;    //九转大肠

            case "SpicyBase": //麻辣烫基底
                int spicyCount = potManager.CountTagsInPot(TagType.Spicy);
                return spicyCount * 10f;

            case "CompressedBiscuit": //压缩饼干
